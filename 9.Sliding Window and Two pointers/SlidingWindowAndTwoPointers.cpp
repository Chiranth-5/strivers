...
int SlidingWindowAndTwoPointers::totalFruit(vector<char>& fruits)
{
    ...
    // Start at any tree, but as you proceed to the right, 
    // select exactly one fruit from each tree,
    // including the starting tree. One of the baskets must hold the harvested fruits.
    // Once reaching a tree with fruit that cannot fit into any basket, 
    // stop.
    ...
    for ( int right=0; right < n; right++)
    {
        ...
        {
            // not found in basket
            // make space for new fruit so move this by left
            while(basket.size() >= 2 )
            {
